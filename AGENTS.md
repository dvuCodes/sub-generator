...
- For the completion-state `Open Folder` action in Tauri v2, frontend native-path fixes are not sufficient by themselves; keep `src-tauri/tauri.conf.json` `plugins.shell.open` set to a regex that matches native local paths because `true` still falls back to the plugin's default URL-only validator. Pair that with a plain `"shell:allow-open"` capability instead of a `file:///*`-scoped workaround.
